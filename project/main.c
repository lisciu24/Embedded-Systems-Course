...
void EINT0_IRQHandler(void) {
  ...
  UART_write_string("Click!\r\n");
	if (dma) DMA_stop(); else DMA_init(values_idx);
...
void graph(void) {
    ...
    fill_rect(ERASE_B_MIN_X, ERASE_B_MIN_Y, ERASE_B_MAX_X - ERASE_B_MIN_X, ERASE_B_MAX_Y - ERASE_B_MIN_Y, LCDMagenta);// erase button
    const char *erase_str = "ERASE";
    draw_text(erase_str, ERASE_B_MIN_X + 4, ERASE_B_MIN_Y + 5, LCDBlack);
    fill_rect(FIX_B_MIN_X, FIX_B_MIN_Y, FIX_B_MAX_X - FIX_B_MIN_X, FIX_B_MAX_Y - FIX_B_MIN_Y, LCDGreen);// fix the graph button
