...
void draw_char_vertical(uint8_t chr, uint16_t x, uint16_t y, uint16_t color,
                        uint16_t bg_color) {
    // char is 8x16 (w x h)
    // top to bottom line by line
    uint8_t ascii_char[16];
...
void draw_text_vertical(const char *str, uint16_t x, uint16_t y, uint16_t color,
                        uint16_t bg_color) {
    ...
    // Code below from init_ILI9235 in LCD_ILI9325.h
    // ### lcdWriteReg(0x03, 0x1030); set GRAM write direction and BGR=1 ###
    uint16_t default_entry = 0x1030;

    // Settings for lcd vertical writing
    // vertical axis is main axis
    uint16_t am = (1 << 3);
    // 11 means horizontal increment and vertical increment
    uint16_t id = (0b11 << 4);

    uint16_t this_entry = (default_entry & ~(0b111 << 3)) | am | id;
