...
void init_Timer1(void) {
  ...
  // config for timing 100 ms
  LPC_TIM1->PC = 1e5 - 1;      // Set prescaler to 10^5
  LPC_TIM1->MR0 = 25;          // Set match 0 to 25
...
void TIMER1_IRQHandler(void) {
  ...
  LPC_GPIO2->FIOPIN0 ^=
      (diodes[diodes_idx] ^ diodes[next_idx]); // Switch diodes to new pattern
...
void init_LLI(void) {
  ...
  LLI.DMACCControl = 0;              // Clear whole control register
  LLI.DMACCControl |=
      diodes_size; // Set transfer size -  how many elements will be transfered
  LLI.DMACCControl |= 1 << 26; // Turn on source address increment
}
...
void init_DMA(void) {
  ...
  LPC_GPDMACH0->DMACCControl = 0; // Clear whole control register
  LPC_GPDMACH0->DMACCControl |=
      diodes_size; // Set transfer size -  how many elements will be transfered
  LPC_GPDMACH0->DMACCControl |= 1 << 26; // Turn on source address increment
  LPC_GPDMACH0->DMACCControl |=
      1 << 31;                   // Enable interrupt on transfer completion
  LPC_GPDMACH0->DMACCConfig = 0; // Clear whole config register
  LPC_GPDMACH0->DMACCConfig |=
      0x0A << 6; // Set destination peripherial to Timer 1 match 0
  LPC_GPDMACH0->DMACCConfig |=
      0b001 << 11; // Set transfer type to memory to peripherial
  LPC_GPDMACH0->DMACCConfig |=
      0b11 << 14; // Disable mask for error and terminal interrupt
